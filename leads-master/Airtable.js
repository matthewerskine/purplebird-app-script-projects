...
const SHEET_COL_VERIFY_COMPANY_STATUS_COMPANY_NUMBER =
  "verifyCompanyStatus.companyNumber";
const SHEET_COL_VERIFY_COMPANY_STATUS_TRADING_STATUS =
  "verifyCompanyStatus.tradingStatus";
const SHEET_COL_VERIFY_COMPANY_STATUS_CONFIDENCE =
  "verifyCompanyStatus.confidence";
...
function sendSelectedLeadsToAirtable() {
  ...
  if (!AIRTABLE_API_KEY || !AIRTABLE_BASE_ID) {
    SpreadsheetApp.getUi().alert(
      "Configuration Error",
      "Airtable API Key or Base ID is missing. Please set them in Project Properties > Script Properties.",
  ...
  if (sheet.getName() !== SHEET_QUALIFY) {
    ui.alert(
      "Wrong Sheet",
      `This function should only be run from the "${SHEET_QUALIFY}" sheet.`,
      ui.ButtonSet.OK
    );
    return;
  }
  ...
  for (const col of requiredSheetCols) {
    if (headerMap[col] === undefined) {
      ui.alert(
        ...
        `A required column "${col}" was not found in Row 1 of your sheet.`,
        ui.ButtonSet.OK
      );
      return;
    }
  ...
  if (!dataRange || dataRange.isBlank()) {
    ui.alert(
      "No Data Selected",
      "Please select the data rows you want to send to Airtable.",
      ui.ButtonSet.OK
    );
    return;
  }
  ...
  if (dataRange.getRow() === 1) {
    if (dataRange.getNumRows() > 1) {
      ...
      ui.alert(
        "Header Row Selected",
        "Please select data rows below Row 1.",
        ui.ButtonSet.OK
      );
      return;
    }
...
function processDataRows(sheet, dataRowsRange, headerMap) {
  ...
  for (let i = 0; i < dataRowsToProcess.length; i++) {
    ...
    if (airtableActionColIndex !== undefined) {
      const actionValue = String(rowData[airtableActionColIndex] || "")
        .trim()
        .toLowerCase();
      ...
      if (actionValue === "skip") {
        ...
        Logger.log(
          `Row ${actualSheetRowIndex}: Skipping due to 'skip' command in airtableAction column.`
        );
    ...
    if (
      processedStatusCell &&
      (String(processedStatusCell).toLowerCase().startsWith("sent") ||
        String(processedStatusCell).toLowerCase().startsWith("verified"))
    ) {
    ...
    if (!companyName) {
      Logger.log(
        `Sheet Row ${actualSheetRowIndex}: Skipping due to missing company name.`
      );
      failCount++;
      continue;
    }
    ...
    const airtableRecord = {
      fields: {
        ...
        [AIRTABLE_FIELD_PHONE]:
          headerMap[SHEET_COL_PHONE] !== undefined
            ? String(rowData[headerMap[SHEET_COL_PHONE]] || "")
            : null,
        [AIRTABLE_FIELD_WEBSITE_URL]:
          headerMap[SHEET_COL_WEBSITE] !== undefined
            ? rowData[headerMap[SHEET_COL_WEBSITE]]
            : null,
        [AIRTABLE_FIELD_INDUSTRY]:
          headerMap[SHEET_COL_INDUSTRY] !== undefined
            ? rowData[headerMap[SHEET_COL_INDUSTRY]]
            : null,
    ...
    if (headerMap[SHEET_COL_EMAIL_EXTRACTED] !== undefined) {
      airtableRecord.fields[AIRTABLE_FIELD_EMAIL] =
        rowData[headerMap[SHEET_COL_EMAIL_EXTRACTED]] || null;
    }

    if (headerMap[SHEET_COL_ADS_RUNNING] !== undefined) {
      const sheetValue = rowData[headerMap[SHEET_COL_ADS_RUNNING]];
      airtableRecord.fields[AIRTABLE_FIELD_ADS_RUNNING] =
        String(sheetValue || "")
          .trim()
          .toLowerCase() === "yes";
    }
    ...
    for (const key in airtableRecord.fields) {
      if (
        airtableRecord.fields[key] === null ||
        airtableRecord.fields[key] === undefined
      ) {
    ...
    recordsToCreate.push({
      ...
      sheetRowIndex: actualSheetRowIndex,
    });
  }
  ...
  if (recordsToCreate.length === 0) {
    ...
    if (alreadyProcessedCount > 0 || skippedByActionCount > 0) {
      alertMessage =
        "No new leads found to send.\n" +
        (alreadyProcessedCount > 0
          ? `${alreadyProcessedCount} lead(s) were already processed/verified.\n`
          : "") +
        (skippedByActionCount > 0
          ? `${skippedByActionCount} lead(s) were marked as 'skip'.`
          : "");
    }
    ...
    if (response && response.records) {
      response.records.forEach((createdRecord, index) => {
        ...
        if (createdRecord.id) {
          ...
          sheet
            .getRange(
              originalItem.sheetRowIndex,
              headerMap[SHEET_COL_PROCESSED] + 1
            )
            .setValue(`Sent on ${todayISO} (ID: ${createdRecord.id})`);
        } else {
          failCount++;
          sheet
            .getRange(
      ...
      batch.forEach((originalItem) => {
        sheet
          ...
          .setValue(`API Error on ${todayISO}`);
      });
    }
  }
  ...
  let message =
    ...
    (alreadyProcessedCount > 0
      ...
      : "") +
    (skippedByActionCount > 0
      ? `Skipped (marked as 'skip'): ${skippedByActionCount}\n`
      : "");
...
function safeDiagnoseRow2706() {
  ...
  try {
    ...
    Logger.log(
      `Column letter: ${
        airtableActionColIndex !== undefined
          ? String.fromCharCode(65 + airtableActionColIndex)
          : "NOT FOUND"
      }`
    );
    ...
    if (rawActionValue) {
      Logger.log(
        `Character codes: ${Array.from(rawActionValue)
    ...
    const actionValue = String(rawActionValue || "")
      .trim()
      .toLowerCase();
    Logger.log(`\n=== LOGIC ANALYSIS ===`);
    ...
    Logger.log(
      `actionValue.charCodeAt(0): ${
        actionValue.length > 0 ? actionValue.charCodeAt(0) : "N/A"
      }`
    );
  ...
  try {
    ...
    if (airtableActionColIndex !== undefined) {
      ...
      testRows.forEach((rowData, i) => {
        ...
        Logger.log(
          `Row ${rowIndex}: "${rawActionValue}" -> ${
            wouldSkip ? "SKIP" : "PROCESS"
          }`
        );
      });
    } else {
...
function verifyLeadsInAirtable() {
  ...
  if (sheet.getName() !== SHEET_QUALIFY) {
    ui.alert(
      "Wrong Sheet",
      `This function must be run from the "${SHEET_QUALIFY}" sheet.`,
      ui.ButtonSet.OK
    );
    return;
  }
  ...
  for (const col of requiredCols) {
    if (headerMap[col] === undefined) {
      ui.alert(
        "Missing Header Column",
        `A required column "${col}" was not found in Row 1.`,
        ui.ButtonSet.OK
      );
      return;
    }
  ...
  ui.showModalDialog(
    HtmlService.createHtmlOutput(
      "<p>Verifying leads with Airtable... Please wait.</p>"
    ),
  ...
  for (let i = 0; i < dataRowsToVerify.length; i++) {
    ...
    if (
      rowData[processedColIdx] &&
      (String(rowData[processedColIdx]).toLowerCase().startsWith("sent") ||
        String(rowData[processedColIdx]).toLowerCase().startsWith("verified"))
    ) {
      skippedCount++;
      continue;
    }
    ...
    try {
      ...
      if (response && response.records && response.records.length > 0) {
        ...
        sheet
          .getRange(actualSheetRowIndex, processedColNum)
          .setValue(`Verified on ${todayISO} (ID: ${airtableId})`);
      } else if (response) {
        notFoundCount++;
      } else {
        errorCount++;
      ...
      sheet
        .getRange(actualSheetRowIndex, processedColNum)
        .setValue(`Script Error on ${todayISO}`);
    }
  }
...
function debugSelectedLeadsToAirtable() {
  ...
  if (sheet.getName() !== SHEET_QUALIFY) {
    Logger.log(
      `❌ WRONG SHEET: Expected "${SHEET_QUALIFY}", got "${sheet.getName()}"`
    );
    ui.alert(
      "Wrong Sheet",
      `This function should only be run from the "${SHEET_QUALIFY}" sheet.`,
  ...
  for (let i = 0; i < dataRowsToProcess.length; i++) {
    ...
    if (airtableActionColIndex !== undefined) {
      ...
      if (actionValue === "skip") {
        skippedByActionCount++;
        Logger.log(
          `✅ ROW ${actualSheetRowIndex}: SKIPPED due to 'skip' command`
        );
        continue; // Go to the next row immediately
      } else {
        Logger.log(
          `➡️ ROW ${actualSheetRowIndex}: NOT SKIPPED - action value is not 'skip'`
        );
      }
    } else {
      Logger.log(
        `⚠️ ROW ${actualSheetRowIndex}: No airtableAction column found`
      );
    }
    ...
    if (!companyName) {
      Logger.log(
        `❌ ROW ${actualSheetRowIndex}: SKIPPED - missing company name`
      );
      failCount++;
      continue;
    }
...
function identifyBadRecords() {
  ...
  console.log(
    `✅ Using airtableAction column at index ${airtableActionColIndex}`
  );
  ...
  for (let i = 1; i < allData.length; i++) {
    ...
    if (actionValue === "skip" && processedStatus.startsWith("Sent on")) {
      ...
      if (idMatch) {
        ...
        console.log(
          `  ⚠️ Could not extract Airtable ID from: "${processedStatus}"`
        );
      }
    }
  }
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    console.log(
      `\nChecking ${i + 1}/${candidates.length}: "${
        candidate.companyName
      }" (ID: ${candidate.airtableId})`
    );
    ...
    try {
      ...
      if (hasActivity) {
        console.log(
          `  ⚠️ HAS ACTIVITY: Cannot delete - agent has used this lead`
        );
        hasActivities.push(candidate);

        // Update Airtable Source field to "GM - Unqualified"
        console.log(
          '  📝 Updating Airtable Source field to "GM - Unqualified"...'
        );
        const updatePayload = {
          fields: {
        ...
        const updateResponse = callAirtableApi(
          "PATCH",
          updatePayload,
          candidate.airtableId
        );
        if (updateResponse) {
        ...
        sheet
          .getRange(candidate.sheetRow, processedColIndex + 1)
          .setValue(newStatus);
        console.log(
          `  📝 Updated sheet row ${candidate.sheetRow} to "In Pipeline"`
        );
      } else {
  ...
  if (toDelete.length > 0) {
    ...
    toDelete.forEach((record, index) => {
      console.log(
        `${index + 1}. Row ${record.sheetRow}: "${record.companyName}" (ID: ${
          record.airtableId
        })`
      );
    });
    ...
    console.log(
      "3. Each record will be deleted individually with verification"
    );
  }
  ...
  PropertiesService.getScriptProperties().setProperty(
    "BAD_RECORDS_TO_DELETE",
    JSON.stringify(toDelete)
  );
  PropertiesService.getScriptProperties().setProperty(
    "BAD_RECORDS_HAS_ACTIVITIES",
    JSON.stringify(hasActivities)
  );
  PropertiesService.getScriptProperties().setProperty(
    "BAD_RECORDS_NOT_FOUND",
  ...
  if (notFound.length > 0) {
    ...
    for (const record of notFound) {
      ...
      sheet
        .getRange(record.sheetRow, processedColIndex + 1)
        .setValue(newStatus);
      console.log(
        `  📝 Updated row ${record.sheetRow}: "${record.companyName}"`
      );
    }
    console.log(
      `✅ Updated ${notFound.length} sheet rows for "not found" records`
    );
  }
}
...
function identifyBadRecordsBatch(batchSize = 10) {
  ...
  console.log(
    "This will process records in smaller batches to avoid timeouts."
  );
  console.log("");

  // Get the list of records to delete from properties
  const toDeleteJson = PropertiesService.getScriptProperties().getProperty(
    "BAD_RECORDS_TO_DELETE"
  );
  if (!toDeleteJson) {
    console.log(
      "❌ ERROR: No records to delete found. Run identifyBadRecords() first."
    );
    return;
  }
  ...
  console.log(
    `Processing records ${startIndex + 1} to ${endIndex} of ${
  ...
  for (let i = 0; i < batch.length; i++) {
    ...
    console.log(
      `\nProcessing ${startIndex + i + 1}/${allCandidates.length}: "${
        candidate.companyName
      }"`
    );
  ...
  if (processed.length >= allCandidates.length) {
    ...
    console.log(
      `Run identifyBadRecordsBatch() again to process the next ${batchSize} records.`
    );
  }
}
...
function updatePipelineLeadsSource() {
  ...
  console.log(
    'This will update the Source field to "GM - Unqualified" for leads that:'
  );
  console.log('1. Have "skip" in airtableAction column');
  console.log('2. Have "Sent on ..." in processed column');
  ...
  const hasActivitiesJson = PropertiesService.getScriptProperties().getProperty(
    "BAD_RECORDS_HAS_ACTIVITIES"
  );
  if (!hasActivitiesJson) {
    console.log(
      "❌ ERROR: No pipeline leads found. Run identifyBadRecords() first."
    );
    return;
  }
  ...
  for (let i = 0; i < hasActivities.length; i++) {
    ...
    console.log(
      `Row ${record.sheetRow}: "${record.companyName}" (ID: ${record.airtableId})`
    );
    ...
    try {
      ...
      if (updateResponse) {
        console.log(
          `  ✅ Successfully updated Source field to "GM - Unqualified"`
        );
        successCount++;
      } else {
  ...
  if (successCount > 0) {
    ...
    console.log(
      'Pipeline leads are now marked as "GM - Unqualified" for reporting purposes.'
    );
  } else {
    console.log(
...
function deleteBadRecords() {
  ...
  console.log(
    "This function will delete records one at a time with full verification."
  );
  console.log("");
  ...
  console.log("");
  console.log(
    "⚠️ WARNING: This will permanently delete these records from Airtable."
  );
  ...
  for (let i = 0; i < toDelete.length; i++) {
    ...
    try {
      ...
      const newStatus = `Deleted from Airtable on ${new Date()
        .toISOString()
        .slice(0, 10)} (was: ${record.processedStatus})`;
      sheet
        .getRange(record.sheetRow, processedColIndex + 1)
        .setValue(newStatus);
...
function checkColumnNames() {
  ...
  websiteVariations.forEach((variation) => {
    if (headerMap[variation] !== undefined) {
      console.log(
        `✅ FOUND: "${variation}" at column ${String.fromCharCode(
          65 + headerMap[variation]
        )}`
      );
    } else {
...
function deleteBadRecordsTest() {
  ...
  console.log(
    `Row ${testRecord.sheetRow}: "${testRecord.companyName}" (ID: ${testRecord.airtableId})`
  );
  console.log("");
  ...
  try {
    ...
    // Step 3: Delete from Airtable
    console.log("Step 2: Deleting from Airtable...");
    const deleteResponse = callAirtableApi(
      "DELETE",
      null,
      testRecord.airtableId
    );
    ...
    const newStatus = `Deleted from Airtable on ${new Date()
      .toISOString()
      .slice(0, 10)} (was: ${testRecord.processedStatus})`;
    sheet
      .getRange(testRecord.sheetRow, processedColIndex + 1)
      .setValue(newStatus);
    ...
    console.log(
      "If this worked correctly, you can now run deleteBadRecords() for all records."
    );
  } catch (error) {
    console.log(`  ❌ ERROR: ${error.message}`);
...
function sendSelectedLeadsToAgentSheet() {
  ...
  if (!dataRange || dataRange.isBlank()) {
    ui.alert(
      "No Data Selected",
      "Please select the data rows you want to send to the agent sheet.",
      ui.ButtonSet.OK
    );
    return;
  }
  ...
  const response = ui.prompt(
    "Target Sheet",
    "Please enter the Google Sheets URL for the agent sheet:",
 ...
 */
function processDataRowsToAgentSheet(
  sheet,
  dataRowsRange,
  headerMap,
  targetUrl
  ...
  for (let i = 0; i < dataRowsToProcess.length; i++) {
    ...
    if (
      processedStatusCell &&
      String(processedStatusCell).toLowerCase().includes("sent to agent sheet")
    ) {
      alreadyProcessedCount++;
      continue;
    }
    ...
    const success = copyRowToTargetSheet(
      rowData,
      headerMap,
      targetSheet,
      targetHeaderMap,
  ...
  let message =
    ...
    (failCount > 0 ? `Failed: ${failCount}\n` : "") +
    (alreadyProcessedCount > 0
      ? `Skipped (already sent to agent sheet): ${alreadyProcessedCount}\n`
      : "") +
    (skippedByActionCount > 0
      ? `Skipped (marked as 'skip'): ${skippedByActionCount}\n`
      : "");
  ...
  if (successCount > 0) {
    sendSlackNotification(
      `✅ Agent sheet transfer completed: ${successCount} records transferred successfully`
    );
  }
  if (failCount > 0) {
...
function validateAndOpenTargetSheet(url) {
  try {
    ...
    if (!spreadsheetId) {
      return {
        error:
          "Invalid Google Sheets URL format. Please use a URL like: https://docs.google.com/spreadsheets/d/SPREADSHEET_ID/edit",
      };
    }
    ...
    return {
      error:
        "Cannot access target sheet. Please check the URL and ensure you have permission to access the sheet.",
    };
  }
}
...
function ensureHeadersExist(targetSheet, sourceHeaders) {
  ...
  if (targetSheet.getLastRow() === 0 || targetSheet.getLastColumn() === 0) {
    ...
    targetSheet
      .getRange(1, 1, 1, sourceHeaders.length)
...
function copyRowToTargetSheet(
  ...
  try {
    Logger.log(
      `Copying row with ${Object.keys(targetHeaderMap).length} target columns`
    );

    // Get the maximum column index needed for target sheet
    const targetColumnValues = Object.values(targetHeaderMap);
    const maxTargetColumn =
      targetColumnValues.length > 0 ? Math.max(...targetColumnValues) + 1 : 0;
 ...
 */
function copyCellNotesToTarget(
  sourceSheet,
  sourceRowIndex,
  targetSheet,
  targetRowIndex,
  ...
  try {
    ...
    if (typeof sourceHeaderMap === "number") {
      ...
      for (const [sourceHeader, sourceIndex] of Object.entries(
        sourceHeaderMap
      )) {
        const targetIndex = targetHeaderMap[sourceHeader];
        if (targetIndex !== undefined && sourceNotes[sourceIndex]) {
          const targetCell = targetSheet.getRange(
            targetRowIndex,
            targetIndex + 1
          );
